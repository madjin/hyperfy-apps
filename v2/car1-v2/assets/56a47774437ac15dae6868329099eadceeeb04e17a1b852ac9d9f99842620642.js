...
app.on('fixedUpdate', delta => {
  ...
  // anti-roll
  // only applies to wheels that are directly pointing down on the surface normal (up to 10 degrees)
  const antiRoll = 50
  const fl = wheels[0]
  const fr = wheels[1]
  const fAntiRollForce = (fl.compression - fr.compression) * antiRoll
  if (fl.grounded && fl.groundAngle < 10) {
    const hub = hubs[0]
    const force = _v1.copy(up).multiplyScalar(-fAntiRollForce)
    body.addForceAtLocalPos(force, hub.position)
  }
  if (fr.grounded && fr.groundAngle < 10) {
    const hub = hubs[1]
    const force = _v1.copy(up).multiplyScalar(fAntiRollForce)
    body.addForceAtLocalPos(force, hub.position)
  }
  const bl = wheels[2]
  const br = wheels[3]
  const bAntiRollForce = (bl.compression - br.compression) * antiRoll
  if (bl.grounded && bl.groundAngle < 10) {
    const hub = hubs[2]
    const force = _v1.copy(up).multiplyScalar(-bAntiRollForce)
    body.addForceAtLocalPos(force, hub.position)
  }
  if (br.grounded && br.groundAngle < 10) {
    const hub = hubs[3]
    const force = _v1.copy(up).multiplyScalar(bAntiRollForce)
    body.addForceAtLocalPos(force, hub.position)
  }
