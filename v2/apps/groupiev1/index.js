...
const PIP_BOY_GREEN = '#33ff00'; // Classic Pip-Boy green color
...
Object.keys(STATS_CONFIG).forEach((stat, index) => {
    ...
    bar.width = 290;
    bar.height = 20;
    // Use red for health bar, Pip-Boy green for others
    bar.backgroundColor = stat === 'health' ? '#ff0000' : PIP_BOY_GREEN;
    container.add(bar);
...
function findNearestItem(preferredType = null) {
    let nearest = null;
    let minDistance = Infinity;
    
    for (const [id, object] of objectState.perceivedObjects) {
        if (!preferredType || object.type === preferredType) {
...
function updateAnimation(newAnimation) {
    if (objectState.currentAnimation !== newAnimation) {
        ...
        // Get animation URL based on current state
        let animUrl;
        switch (newAnimation) {
...
app.on('update', delta => {
    ...
    if (objectState.isMoving) {
        ...
        if (objectState.followTarget) {
            ...
            // If we're closer than minimum distance, switch to idle animation and stay in place
            if (distanceToPlayer < MIN_FOLLOW_DISTANCE) {
                updateAnimation(ANIMATIONS.IDLE);
                // Normalize direction and set target position at minimum distance
                ...
                updateAnimation(ANIMATIONS.WALK);
                targetPosition = objectState.followTarget.position;
            }
            updateTargetText(`Following: ${objectState.followTarget.name}`);
        } else {
            // Check for items based on search preference
            const nearestItem = findNearestItem(objectState.searchType);
            if (nearestItem) {
        ...
        if (distance < 0.1) {
            ...
            const moveAmount = MOVEMENT_SPEED * delta;
            ...
            const targetAngle = Math.atan2(direction.x, direction.z) + Math.PI; // Add Math.PI (180 degrees)
    ...
    world.on('chat', msg => {
        ...
        // Process commands
        if (CHAT_COMMANDS.COME.some(cmd => command.includes(cmd))) {
        ...
        if (CHAT_COMMANDS.SEARCH.some(cmd => command.includes(cmd))) {
            if (command.includes('health')) {
                objectState.searchType = 'health';
            } else if (command.includes('loot')) {
                objectState.searchType = 'loot';
            }
            objectState.isMoving = true;
            triggerPerception();
            respond(RESPONSES.SEARCHING);
            responded = true;
        }
        
        if (CHAT_COMMANDS.PATROL.some(cmd => command.includes(cmd))) {
            objectState.followTarget = null;
            objectState.searchType = null;
            objectState.isMoving = true;
            targetPosition = generateRandomPosition();
        ...
        if (CHAT_COMMANDS.STATUS.some(cmd => command.includes(cmd))) {
            const response = getRandomResponse(RESPONSES.STATUS)
                .replace('{health}', Math.round(objectState.stats.health))
                .replace('{loot}', Math.round(objectState.stats.loot));
            respond(response);
            responded = true;
        }
        
        if (CHAT_COMMANDS.HELP.some(cmd => command.includes(cmd))) {
