...
function findNearestItem() {
    ...
    for (const [id, object] of objectState.perceivedObjects) {
        if (object.type === 'health' || object.type === 'cargo') {
...
world.on('item-detected', (info) => {
    if (info.collectorId === app.instanceId) {
        ...
        } else if (info.item.type === 'cargo') {
            objectState.stats.cargo = Math.min(
                STATS_CONFIG.cargo.max,
                objectState.stats.cargo + info.item.value
            );
            updateStatDisplay('Cargo');
        }
